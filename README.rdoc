...
upload progress when a request comes by which includes the
+upload_progress_id+ parameter in the query-string (on the URL).  The
other handler will provide the status of a given +upload_progress_id+
in JSON format.
...
The provided JavaScript will look for a form with encoding type
+multipart/form-data+ and an input tag of type +file+ inside and
decorate it to use upload progress.  Note the progress bar will kick
in when the submit takes longer than 2 seconds.
...
an upload progress bar.  If such a form is found an iframe are added.
The form is altered slightly by adding an extra parameter with an uniq
identifier to the form action, setting the form target to the
